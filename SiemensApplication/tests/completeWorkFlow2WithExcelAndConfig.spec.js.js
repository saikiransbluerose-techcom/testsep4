...
test('RiskStage submit + approvals (no filter; open via Recents)', async ({ page }) => {
    ...
    test.slow();  
    test.setTimeout(10 * 60 * 1000); // 10 minutes for this end-to-end
  ...
  await u0.gadget1.getByLabel('Is the cause known?').selectOption(getSW('causeKnown2'));
  await u0.gadget1.getByRole('textbox', { name: 'Lastname, Firstname,' }).click();
  await u0.gadget1.getByRole('textbox', { name: 'Lastname, Firstname,' }).fill(getAssigneeId(5));
  await u0.gadget1.getByText(getAssigneeName(5)).click();
  ...
  await u0.gadget1.getByRole('textbox', { name: 'Root cause analysis' }).fill(getSW('rootCauseAnalysis'));
...
// Locators
const stageHeader = u0.gadget1.getByRole('heading', { name: 'Root cause analyze', exact: true });
const stageBlock  = u0.gadget1.locator('[data-test-id="201810251843350638136498"]');
...
await stageBlock.evaluate(el => { el.style.outline = ''; });
  ...
  await u0.gadget1.getByTestId('201909111935010721260-R3-L1R1').getByTestId('20151130155332032025152').click();
  await u0.gadget1.getByTestId('201909111935010721260-R3-L1R1').locator('i').nth(1).click();
  ...
  await u0.gadget1.getByTestId('201909111935010721260-R3-L1R2').getByTestId('20151130155332032025152').click();
  await u0.gadget1.getByTestId('201909111935010721260-R3-L1R2').locator('i').nth(1).click();
...
// --- handle possible banner refresh (inline) ---
const banner = page.getByTestId('202108040831080324727'); // "Reload this page..."
if (await banner.isVisible().catch(() => false)) {
  await banner.click();
  await page.waitForTimeout(300); // tiny debounce for section refresh
}

// 1) ensure the control that reveals the email field is present
const notifyToggle = u0.gadget1.getByTestId('201609080744500837333');
await expect(notifyToggle).toBeVisible({ timeout: 30000 });

// 2) force it ON (use check(); fall back to click() if it's not an input)
try {
  await notifyToggle.check({ timeout: 10000 });
} catch {
  await notifyToggle.click();
}

// 3) the toggle can trigger an in-place refresh; handle it
if (await banner.isVisible().catch(() => false)) {
  await banner.click();
  await page.waitForTimeout(300);
}

// 4) wait for the email input to attach; retry once if needed
let emailInput = u0.gadget1.getByTestId('20160908075844034240762');
try {
  await emailInput.waitFor({ state: 'attached', timeout: 30000 });
} catch {
  // retry path if first refresh wasn't done or toggle collapsed
  await notifyToggle.click();
  if (await banner.isVisible().catch(() => false)) {
    await banner.click();
    await page.waitForTimeout(300);
  }
  emailInput = u0.gadget1.getByTestId('20160908075844034240762');
  await emailInput.waitFor({ state: 'attached', timeout: 30000 });
}

// now ensure it's interactable and fill
await expect(emailInput).toBeVisible({ timeout: 30000 });
await expect(emailInput).toBeEditable({ timeout: 30000 });
await emailInput.fill(getAssigneeName(4)); // or your static email
...
await u0.gadget1.getByRole('button', { name: 'Submit workflow' }).click();
  ...
  await u0.gadget1.getByTestId('202203310845310836825').check();
  await u0.gadget1.getByTestId('2016091407195807821936').fill('Notify');
  await u0.gadget1.getByTestId('202203211023310169879').click();
  ...
  await u0.gadget1.getByRole('button', { name: 'Submit' }).click();
  await u0.gadget1.getByRole('button', { name: 'Save & Send Comment' }).click();
...
// clicks the reload banner if present, then returns a fresh gadget-1 frame locator
async function ensureFreshGadget(page) {
  const banner = page.getByTestId('202108040831080324727'); // "Reload this page..."
  if (await banner.isVisible().catch(() => false)) {
    await banner.click();
    await page.waitForLoadState('domcontentloaded');
  }
  return page.frameLocator('iframe[name="PegaGadget1Ifr"]');
}

// Clicks the reload banner if present; does NOT return a locator
async function maybeReload(page) {
  const banner = page.getByTestId('202108040831080324727'); // "Reload this page..."
  if (await banner.isVisible().catch(() => false)) {
    await banner.click();
    // Pega section refresh is partial; a small debounce is safer than waitForLoadState
    await page.waitForTimeout(300);
  }
}
